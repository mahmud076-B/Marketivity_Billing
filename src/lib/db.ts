...
if (!databaseUrl && typeof process !== "undefined" && process.env.NODE_ENV === "production") {
  throw new Error(
    "DATABASE_URL is not set in production. The application must not fall back to embedded PGlite in production. Please configure DATABASE_URL in your production environment.",
  );
}
...
type TxRunner = <R>(fn: (tx: Sql) => Promise<R>) => Promise<R>;
...
function toSql(run: Run, transaction?: TxRunner): Sql {
  ...
  sql.query = <T = Record<string, unknown>>(text: string, params: unknown[] = []) =>
    run<T>(text, params);
  sql.transaction = transaction ?? (async (fn) => fn(sql));
  return sql;
}
...
function createNeonSql(): Promise<Sql> {
  globalRef.__pgSqlPromise__ ??= (async () => {
    ...
    const txRunner: TxRunner = async <R>(fn: (tx: Sql) => Promise<R>): Promise<R> => {
      const client = await pool.connect();
      try {
        await client.query("BEGIN");
        const clientRun = async <T>(text: string, params: unknown[]) => {
          const res = await client.query(text, params);
          return res.rows as T[];
        };
        const txSql = toSql(clientRun, async (nested) => nested(txSql));
...
async function createPgliteSql(): Promise<Sql> {
  ...
  // Uses persistent local dataDir when in Node environment, ensuring data survives restarts.
  globalRef.__pgliteInstance__ ??= (async () => {
    ...
    const dataDir =
      ...
      (typeof process !== "undefined" && process.env.NODE_ENV !== "test" ? ".data/pglite" : undefined);
  ...
  const txRunner: TxRunner = async <R>(fn: (tx: Sql) => Promise<R>): Promise<R> => {
    return await pg.transaction(async (tx) => {
      ...
      const txSql = toSql(txRun, async (nested) => nested(txSql));
