...
import { Pool } from "pg";
import { ensureDbReady, getPglite, getSql } from "../db.ts";
    ...
    user: {
      create: {
        before: async (user) => {
          ...
          const token = getCookie("invite_token");
          
          if (!token) {
            throw new Error("Registration is closed. Valid invitation token required.");
          }
          ...
          if (rows.length === 0) {
            throw new Error("Registration is closed. Valid invitation not found or already used.");
          }
