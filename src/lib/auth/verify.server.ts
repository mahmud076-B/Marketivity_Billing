...
import { UnauthorizedError, type VerifiedUser } from "../server/authz.ts";
export { UnauthorizedError, type VerifiedUser };
...
export async function getSessionUser(
  ...
  return { 
    ...
    // Better Auth schema extension provides these
...
export async function requireUser(bearerToken?: string): Promise<VerifiedUser> {
  if (!authConfigured && !gateIdentityEnabled()) {
    ...
    return { id: DEV_USER_ID, email: null, role: "admin", status: "active" };
  }
