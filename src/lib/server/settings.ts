...
export const getSettings = createServerFn({ method: "GET" })
  ...
  .handler(async ({ context }) => {
    ...
    const rows = await sql`select * from settings where user_id = ${context.userId}`;
  ...
  .handler(async ({ context, data }) => {
    ...
    await sql`
      insert into settings (
        ...
        ${context.userId}, ${s.agencyName}, ${s.tagline}, ${s.positioning}, ${s.phone}, ${s.whatsapp},
...
export const clearSampleData = createServerFn({ method: "POST" })
  ...
  .handler(async ({ context }) => {
    ...
    await sql`delete from payments where user_id = ${uid} and invoice_id in (select id from invoices where user_id = ${uid} and is_sample = true)`;
    await sql`delete from invoice_items where user_id = ${uid} and invoice_id in (select id from invoices where user_id = ${uid} and is_sample = true)`;
    await sql`delete from invoices where user_id = ${uid} and is_sample = true`;
    await sql`delete from clients where user_id = ${uid} and is_sample = true`;
    await sql`delete from services where user_id = ${uid} and is_sample = true`;
    return { ok: true };
  });
